...
void loop(){
  ...
  if(error == 0){
    ...
    Serial.println(F("==================================================="));
    int status = IMU.begin();             //Start the I2C as master
    if (status < 0) {
...
void gyro_signalen(){
  IMU.readSensor();
  gyro_roll = IMU.getGyroX_rads();                   //Read high and low part of the angular data.
  if(cal_int == 2000)gyro_roll -= gyro_roll_cal;               //Only compensate after the calibration
  gyro_pitch = IMU.getGyroY_rads();                   //Read high and low part of the angular data.
  if(cal_int == 2000)gyro_pitch -= gyro_pitch_cal;             //Only compensate after the calibration
  gyro_yaw = IMU.getGyroZ_rads();                   //Read high and low part of the angular data.
